...
- Removal can be queued while a node is offline. It replaces operations that have not started and waits for any operation already leased to the agent. The workspace and its reservations remain until the agent reconnects with Docker available and confirms removal; its persistent volume is retained.
- A workspace on a revoked node has a separate **Remove workspace record** action. This removes its coordinator metadata and cancels pending jobs; it cannot remove any Docker container or volume on the revoked node. Clean up those resources locally.
